...
with open('support/databases/airports.pkl', 'rb') as fp:
...
def all_destinations(destinations):
    all_destinations = []
    if destinations['amadeus_direct'] != {}:
...
def get_optimal_ticket(tickets,
                       request_params, budget_tag
                       ):
    ...
    activity_tag = request_params['activity_tag']
    ...
    result = tickets_weights(final_tickets=final_tickets, activity_tag=activity_tag, budget_tag=budget_tag,
...
def pack_data_files_into_json(data_directory, session_id):
    # List of data files you want to combine
    data_files = [f'live_offer1_{session_id}.json', 'offerData2.json', 'offerData3.json']
    # data_files = ['offerData1.json', 'offerData2.json', 'offerData3.json']
    combined_data = {}

    for offer_id, filename in enumerate(data_files, start=1):
        file_path = os.path.join(data_directory, filename)
        with open(file_path, 'r') as file:
            data = json.load(file)
            # Tagging each offer's data with an offerId
            combined_data[f"offerId_{offer_id}"] = data

    # Optionally, if you want to write the combined data to a new JSON file
    # output_file_path = os.path.join(data_directory, 'combined_offers.json')
...
def get_raw_offer_data(data):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        json_data = json.dumps({'flight_tickets': tickets.result(), 'all_hotels': hotels.result()})
    return json_data

def get_all_tickets(data):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = list(executor.map(raw_tickets, data))
    return results
...
def clear_string(json_string):
    ...
    try:
        cleaned_json_data = json.loads(cleaned_json_string)
        if type(cleaned_json_data) != dict:
            cleaned_json_data = json.loads(cleaned_json_data)
    except json.JSONDecodeError as e:
        print(f"Failed to decode JSON for hotel ID {json_string}: {e}")
